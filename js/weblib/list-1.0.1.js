...
List.LIST_BUILD_DELAY = 1000;

/**
 * v1.0.1 added callbacks to getItemById.
 ...
 * @param id
 * @param onItem. If set, act like callback
 * @returns jquery item
 */
List.prototype.getRootItemById = function (id, onItem) {
    var self = this;
    if (onItem) {
        setTimeout(function () {
            onItem(self.getRootItemById(id));
        }, List.LIST_BUILD_DELAY);
    } else {
 ...
 * after page is changed and items added
 * @param id
 * @param selector - item to find. If undefined, then search span
 * @param onItem. If set, act like callback
 * @returns jquery item
 */
List.prototype.getFirstSubByRootId = function (id, selector, onItem) {
    var self = this;
    var sel = 'span';
    ...
    if (onItem) {
        setTimeout(function () {
            onItem(self.getFirstSubByRootId(id, selector));
        }, List.LIST_BUILD_DELAY);
    } else {
        return this.getRootItemById(id).parent().parent().find(sel).eq(0);
    }
};
 ...
 * @returns jquery item
 */
List.prototype.getSecondSubByRootId = function (id, selector, onItem) {
    var self = this;
    var sel = 'span';
    ...
    if (onItem) {
        setTimeout(function () {
            onItem(self.getSecondSubByRootId(id, selector));
        });
    } else {
        return this.getRootItemById(id).parent().parent().find(sel).eq(1);
    }
};
